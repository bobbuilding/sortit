  ...
  return (
    <div className="p-6 w-full">
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <p className="text-xs mt-1 uppercase tracking-widest text-zinc-500">Immutable audit trail · {transactions.length} records</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-4">
          ...
          <div className="text-2xl font-black text-emerald-400">+${(totalIn/1_000_000).toFixed(2)}M</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 p-4">
          <div className="text-[9px] text-zinc-500 uppercase tracking-widest mb-2">Total Outflows</div>
          <div className="text-2xl font-black text-red-400">-${(totalOut/1_000).toFixed(0)}k</div>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 p-4">
          <div className="text-[9px] text-zinc-500 uppercase tracking-widest mb-2">Net Position</div>
          <div className="text-2xl font-black text-white">+${((totalIn - totalOut)/1_000_000).toFixed(2)}M</div>
        </div>
      </motion.div>
      ...
      <div className="bg-zinc-900 border border-zinc-800">
        <div className="grid grid-cols-[90px_60px_1fr_100px_100px_90px] text-[9px] uppercase tracking-widest text-zinc-600 px-4 py-3 border-b border-zinc-800">
          <span>TXN ID</span><span>Type</span><span>Description</span><span>Node</span><span>Amount</span><span>Status</span>
        </div>
        <motion.div variants={container} initial="hidden" animate="show">
          {filtered.map((txn) => (
            <motion.div key={txn.id} variants={row}
              ...
              <span className="text-[9px] text-zinc-600 font-mono">{txn.node}</span>
              <span className={`text-[11px] font-bold ${txn.amount > 0 ? "text-emerald-400" : "text-red-400"}`}>
                {fmt(txn.amount)}
              </span>
              <span className={`flex items-center gap-1 text-[9px] uppercase ${txn.status === "SETTLED" ? "text-emerald-400/70" : "text-amber-400/70"}`}>
                {txn.status === "SETTLED" ? <CheckCircle size={9}/> : <Clock size={9}/>}
                {txn.status}
              </span>
            </motion.div>
          ))}
